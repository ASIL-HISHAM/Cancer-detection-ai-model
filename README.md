# 🦠 Cancer-detection-ai-model - Early Detection Made Easy
...
3. Ensure you have your environment set up with Conda. If not, install it from the Conda website.
4. Create a new environment (optional but recommended) by running:
   ```
   conda create -n cancer-detection-env python=3.8
   ```
5. Activate your new environment with:
   ```
   conda activate cancer-detection-env
   ```
6. Install necessary packages:
   ```
   conda install pandas
   ```
7. Once everything is set up, you can run the application by typing:
   ```
   python [your_main_file].py
   ```
   Replace `[your_main_file].py` with the actual file name.
